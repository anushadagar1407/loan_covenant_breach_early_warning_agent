...
const registryCopy: Record<RegistryTabId, { title: string; subtitle: string }> = {
  concepts: {
    title: 'Registry overview: the vocabulary before the evidence',
    subtitle:
      'Use this page as the entry point for the evaluation story. It defines the covenant terms, the agent-run terms, and where each kind of evidence lives.',
  },
  metrics: {
    title: 'Registry metrics: how process evidence is scored',
    subtitle:
      'This view defines the five thesis metrics so readers can understand the evidence tables before interpreting them.',
  },
  evidence: {
    title: 'Registry evidence: hidden gaps and autonomy risk',
    subtitle:
      'This view keeps the evidence tables separate from the concept and metric-definition views, so H1 and H2 can be inspected directly.',
  },
}

function ConceptsPanel({ onTabChange }: { onTabChange: (tab: RegistryTabId) => void }) {
  return (
    <>
      ...
      <section className="route-card-grid">
        ...
        />
        <RouteCard
          eyebrow="Evidence tab"
          title="H1/H2 support tables"
          onClick={() => onTabChange('evidence')}
          body="Review hidden process gaps and process-error rates by autonomy level without opening another registry page."
        />
        <RouteCard
...
function MetricsPanel({ onTabChange }: { onTabChange: (tab: RegistryTabId) => void }) {
  return (
    <>
      ...
      <section className="route-card-grid" style={{ marginTop: 20 }}>
        <RouteCard
          eyebrow="Next tab"
          title="Open evidence tables"
          onClick={() => onTabChange('evidence')}
          body="Use the H1/H2 evidence view to see where these metric definitions appear in actual runs."
        />
        <RouteCard
          eyebrow="Back"
          title="Registry concepts"
          onClick={() => onTabChange('concepts')}
          body="Return to the vocabulary view if a reader needs the covenant and agent-run terms first."
        />
      </section>
    </>
  )
...
export default function RegistryPage() {
  ...
  const [activeTab, setActiveTab] = useState<RegistryTabId>('concepts')
  ...
  const copy = registryCopy[activeTab]
  ...
  return (
    <div>
      ...
      <div className="page-content">
        <RegistrySectionNav summary={summary} activeTab={activeTab} onTabChange={setActiveTab} />

        {activeTab === 'concepts' && <ConceptsPanel onTabChange={setActiveTab} />}
        {activeTab === 'metrics' && <MetricsPanel onTabChange={setActiveTab} />}
        {activeTab === 'evidence' && <EvidencePanel summary={summary} h1Data={h1Data} h2Data={h2Data} />}
      </div>
    </div>
  )
}
